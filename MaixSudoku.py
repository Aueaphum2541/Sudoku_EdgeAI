...
def detect_squares(img):
    ...
    blur = gray.gaussian_blur(5)
    edges = blur.canny(50, 150)
    contours = edges.find_contours()

    squares = []
    for contour in contours:
        perimeter = contour.arc_length()
        approx = contour.approx_polygon(0.02 * perimeter)
        
        if len(approx) == 4 and contour.area() > 1000:
...
while True:
    ...
    if squares:
        ...
        for i in range(10):
            for j in range(10):
                img.draw_circle(GridPoints[i][j], 2)

        img.draw_circle(GridPoints[0][0], 10, color=(255, 255, 255))
        img.draw_rectangle(GridPoints[0][0], GridPoints[1][1], color=(255, 0, 0), thickness=2)
    ...
    if lcd.is_pressed(lcd.KEY_RIGHT):
        break
